#!/usr/bin/env python

"""AFR main program."""

from __future__ import print_function

import logging
import sys

import afr.entity
import afr.entitycomponent
import afr.map
import afr.player
import afr.screen
import afr.util

logging.basicConfig(level=logging.DEBUG,
                    format="%(filename)s (%(funcName)s) %(message)s")

MAP_WIDTH = 40
MAP_HEIGHT = 40
ALLOWED_KEYS = set('qhjklyubn')


def _tick(action):
    """Run game turn."""
    for e in afr.entity.entities:
        if e.has_component('player'):
            afr.player.handle_player_action(action, e)
        if e.has_component('ai') and not e.has_component('player'):
            logging.debug("Running AI for %s" % e.name)
            e.run_ai()


def main():
    try:
        # Mapgen
        afr.map.CreateMap(width=MAP_WIDTH, height=MAP_HEIGHT)
        afr.map.map.generate_interior(rooms=5)

        # Test creature init
        coords = [afr.map.map.get_empty_coordinates() for i in range(5)]
        player = afr.entity.Entity('Urist', components=[
            afr.entitycomponent.Creature(max_hp=100),
            afr.entitycomponent.Fighter(strength=5, team='dwarves'),
            afr.entitycomponent.Corporeal(x=coords[0][0], y=coords[0][1],
                                          icon='@', zorder=1),
            afr.entitycomponent.AI(),
            afr.entitycomponent.Inventory(),
            afr.entitycomponent.Player(),
        ]
        )
        afr.entity.entities.add(player)
        afr.entity.entities.add(
            afr.entity.Entity('Goblin King', components=[
                afr.entitycomponent.Creature(max_hp=40),
                afr.entitycomponent.Fighter(strength=10, team='goblins'),
                afr.entitycomponent.Corporeal(x=coords[1][0], y=coords[1][1]),
                afr.entitycomponent.AI(),
            ]
            )
        )
        afr.entity.entities.add(
            afr.entity.Entity(
                'Sword',
                components=[
                    afr.entitycomponent.Corporeal(
                        x=coords[2][0],
                        y=coords[2][1],
                        icon='(',
                        blocks_movement=False,
                        zorder=-1),
                    afr.entitycomponent.Weapon(
                        strength=10),
                ]))
        afr.entity.entities.add(
            afr.entity.Entity('Goblin King', components=[
                afr.entitycomponent.Creature(max_hp=40),
                afr.entitycomponent.Fighter(strength=10, team='goblins'),
                afr.entitycomponent.Corporeal(x=coords[3][0], y=coords[3][1]),
                afr.entitycomponent.AI(),
            ]
            )
        )
        afr.entity.entities.add(
            afr.entity.Entity('Goblin King', components=[
                afr.entitycomponent.Creature(max_hp=40),
                afr.entitycomponent.Fighter(strength=10, team='goblins'),
                afr.entitycomponent.Corporeal(x=coords[4][0], y=coords[4][1]),
                afr.entitycomponent.AI(),
            ]
            )
        )

        # Event loop
        n_tick = 0
        run = True
        while run:
            n_tick += 1
            logging.debug("Tick: %s" % n_tick)
            afr.screen.draw_map(afr.map.map, focus=player)
            action = None
            while not action:
                try:
                    key = raw_input('>_ ')
                except KeyboardInterrupt:
                    print()
                    sys.exit(0)
                action = afr.player.key_to_action(key)
                if not action:
                    logging.warning("Unknown key '%s'", key)
            _tick(action=action)

    except Exception:
        raise
    sys.exit()

if __name__ == '__main__':
    main()
